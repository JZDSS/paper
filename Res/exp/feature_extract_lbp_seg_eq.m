...
for u= 1:6;
	...
	label = [];
	subject_id = [];
	data_folder_name = sprintf('./seg_eyebrow_eq_%d/au%02ddata',sl,k);
...
% 			A0 = A;
% 			E0 = E;
			for m = 1:sl/2:numel(meta)-sl
				fns = m:m+ sl - 1;
				if sum(find(meta(m:m+sl-1) == 1))>sl/2
					label(end+1,1) = 1;
				else
					label(end+1,1) = 0;
				end
...
% 			end
	...
	parsave(data_folder_name,choice,feat_A,feat_E,label,subject_id);
