...
for k = 14%[1 2 4 6 7 10 12 14 15 17 23 24]
	% load label
	load(sprintf('./seg_part/au%02ddata/train_label.mat',k));% true_label = label;
	% load('./data/test_label.mat');  aux_label = label;
	% label = [true_label; aux_label];
	%label = label';
	% load subject id
	load(sprintf('./seg_part/au%02ddata/train_subject_id.mat',k));%    true_subject_id = subject;
	...
	E_feature = 'lbp';
	load(sprintf('./seg_part/au%02ddata/feat_%s_A_%s.mat',k,A_feature,op_norm))
	load(sprintf('./seg_part/au%02ddata/feat_%s_E_%s.mat',k,A_feature,op_norm))
	...
	mkdir(sprintf('./seg_part/au%02ddata/result/cls_acc',k));
	save(sprintf('./seg_part/au%02ddata/result/cls_acc/cls_%s_%s_%s_[%.1f]_f1_[%f].mat',k,f_name,A_feature,op_norm,cr*100,f1),'cr')
end